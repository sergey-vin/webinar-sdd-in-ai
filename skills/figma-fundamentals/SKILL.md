...
Not run against a live server. Confirm: read-tool names match the installed version; **whether `get_design_context`/`get_metadata` return invisible nodes or drop them** (load-bearing — decides whether the extraction-side hidden-text defense works at all, or the render is the sole signal); `get_screenshot` availability for the cross-check; the write-tripwire (`use_figma` refused with a read-scoped PAT) behaves as expected; confirm comments/versions are indeed REST-only so the decline-and-say-so rule is correct. Record in RETRO.
